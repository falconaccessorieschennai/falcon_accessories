...
/**
 * Employee Follow-Ups page.
 * Same as admin but only shows follow-ups created by the current employee.
 * No employee filter needed.
 */

import { useEffect, useState, useRef, FormEvent } from 'react';
import { useAuth } from '@/components/auth/AuthProvider';
import { createFollowUp, getFollowUps, updateFollowUp, deleteFollowUp, FollowUpData } from '@/lib/firestore';
...
export default function EmployeeFollowUpsPage() {
  ...
  async function startRecording() {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        reader.onloadend = () => setFormVoiceNote(reader.result as string);
  ...
  function playVoiceNote(dataUrl: string, id: string) {
    ...
    setPlayingId(id);
    audio.play().catch(() => { showToast('Unable to play voice note.', 'error'); setPlayingId(null); });
  ...
  function openAddForm() {
    ...
    setFormDate(''); setFormCategory('City'); setFormNotes(''); setFormVoiceNote(null); setFormError(null); setShowForm(true);
  }
  ...
  function openEditForm(fu: FollowUpData) {
    ...
    setFormCategory(fu.category); setFormNotes(fu.notes); setFormVoiceNote(fu.voiceNote); setFormError(null); setShowForm(true);
  }
  ...
  async function handleFormSubmit(e: FormEvent) {
    ...
    try {
      const data = {
        ...
        voiceNote: formVoiceNote, notes: formNotes.trim(), createdBy: user?.uid || '', createdByEmail: user?.email || '',
      };
      if (editingId) { await updateFollowUp(editingId, data); showToast('Follow-up updated.', 'success'); }
      else { await createFollowUp(data); showToast('Follow-up added.', 'success'); }
      setShowForm(false);
      loadFollowUps();
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try { await deleteFollowUp(deleteTarget.id); showToast('Follow-up deleted.', 'success'); setDeleteTarget(null); loadFollowUps(); }
  ...
  return (
    <div className="p-6 lg:pl-8 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-text-primary text-2xl font-bold">My Follow-Ups</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {(filterDate || filterCategory) && (
          <button onClick={() => { setFilterDate(''); setFilterCategory(''); }} className="text-text-muted hover:text-text-primary text-sm transition-colors">Clear filters</button>
        )}
      </div>

      {/* List */}
      {loading ? <LoadingSkeleton rows={5} height="h-20" /> : followUps.length === 0 ? (
        <div className="text-center py-16 text-text-muted">
          <p className="text-lg">No follow-ups found.</p>
          <p className="text-sm mt-1">{filterDate || filterCategory ? 'Try adjusting your filters.' : 'Add your first follow-up.'}</p>
        </div>
      ) : (
            ...
            <div className="bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-lg p-6">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className={LABEL_CLASS}>Customer Name *</label><input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} disabled={submitting} className={INPUT_CLASS} placeholder="Customer name" autoFocus /></div>
                  ...
                  <div><label className={LABEL_CLASS}>Fitting Details</label><input type="text" value={formFitting} onChange={(e) => setFormFitting(e.target.value)} disabled={submitting} className={INPUT_CLASS} placeholder="What they want fitted" /></div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    {!recording ? (
                      <button type="button" onClick={startRecording} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium bg-surface-2 border border-border text-text-secondary hover:text-primary hover:border-primary/40 transition-colors"><Mic className="w-4 h-4" />Record</button>
                    ) : (
                      <button type="button" onClick={stopRecording} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium bg-error/10 border border-error/30 text-error transition-colors animate-pulse"><MicOff className="w-4 h-4" />Stop Recording</button>
                    )}
                    {formVoiceNote && (
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => playVoiceNote(formVoiceNote, 'form')} className="flex items-center gap-1 text-xs text-primary hover:text-primary-400 transition-colors"><Play className="w-3 h-3" /> Play</button>
                        <button type="button" onClick={() => setFormVoiceNote(null)} className="text-xs text-text-muted hover:text-error transition-colors">Remove</button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-3 justify-end pt-2">
                  ...
                  <button type="submit" disabled={submitting} className="flex items-center gap-2 bg-primary hover:bg-primary-600 disabled:opacity-60 text-white font-semibold rounded-lg px-5 py-2.5 text-sm transition-colors">{submitting ? 'Saving…' : editingId ? 'Update' : 'Add Follow-Up'}</button>
                </div>
              </form>
            </div>
          </div>
      ...
      {deleteTarget && (
        <ConfirmDialog message={`Delete follow-up for "${deleteTarget.customerName}"?`} confirmLabel={deleting ? 'Deleting…' : 'Delete'} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />
      )}
    </div>
  );
}
